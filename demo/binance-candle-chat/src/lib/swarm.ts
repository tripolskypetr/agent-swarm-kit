...
const PREDICT_SYSTEM_PROMPT = str.newline(
  ...
  `The last 7 days cryptocurrency candles are in first system message of that conversation`,
  "Predict: UP or DOWN (no other information)."
);

const NAVIGATION_SYSTEM_PROMPT = str.newline(
...
History.useHistoryCallbacks({
  getSystemPrompt: async (_, agentName) => {
    if (agentName === AgentName.TriageAgent) {
      return [];
    }
    const c = await candles(AgentCoinMap[agentName]);
    return [`The last 7 days cryptocurrency candles: ${JSON.stringify(c)}`];
  },
});
...
// Navigation Tools
const NAVIGATE_TO_BTC_TRADER = addTool({
  docNote:
...
const NAVIGATE_TO_ETH_TRADER = addTool({
  docNote:
...
const NAVIGATE_TO_BNB_TRADER = addTool({
  docNote:
...
const NAVIGATE_TO_XRP_TRADER = addTool({
  docNote:
...
const NAVIGATE_TO_SOL_TRADER = addTool({
  docNote:
...
addAgent({
  ...
  tools: [
    NAVIGATE_TO_BTC_TRADER,
    NAVIGATE_TO_ETH_TRADER,
    NAVIGATE_TO_BNB_TRADER,
    NAVIGATE_TO_XRP_TRADER,
    NAVIGATE_TO_SOL_TRADER,
  ],
  dependsOn: [
...
addTool({
  ...
  call: async ({ agentName, toolId, clientId, params }) => {
    ...
    await commitToolOutput(
      toolId,
      `To buy the ${coin} for ${params.total} USDT you need to place the order for price=${price} and quantity=${quantity}`,
      clientId,
      agentName
    );
    await emit(
      `To buy the ${coin} for ${params.total} USDT to place the order with price=${price} and quantity=${quantity}`,
      clientId,
      agentName
    );
  },
...
addTool({
  ...
  call: async ({ agentName, toolId, clientId, params }) => {
    ...
    await commitToolOutput(
      toolId,
      `To sell the ${coin} for ${params.total} USDT you need to place the order for price=${price} and quantity=${quantity}`,
      clientId,
      agentName
    );
    await emit(
      `To sell the ${coin} for ${params.total} USDT to place the order with price=${price} and quantity=${quantity}`,
      clientId,
      agentName
    );
  },
