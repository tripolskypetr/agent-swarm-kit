...
const validateClientId = (clientId: string) => {
  ...
  if (!swarm.sessionValidationService.hasSession(clientId)) {
    throw new Error(
      `agent-swarm listenStateEvent session not found for clientId=${clientId}`
    );
  }
};
    ...
    return swarm.busService.subscribe(
      ...
      queued(async (e) => {
        try {
          ...
          console.error(
            `agent-swarm event listener error source=listenStateEvent error=${getErrorMessage(error)}`
          );
        }
      })
    );
