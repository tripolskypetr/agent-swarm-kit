...
const validateClientId = (clientId: string) => {
  ...
  if (!swarm.sessionValidationService.hasSession(clientId)) {
    throw new Error(
      `agent-swarm listenSwarmEventOnce session not found for clientId=${clientId}`
    );
  }
};
...
*/
export const listenSwarmEventOnce = beginContext(
  (
    clientId: string,
    ...
    return swarm.busService.once(
      ...
      queued(async (e) => {
        try {
          ...
          console.error(
            `agent-swarm event listener error source=listenSwarmEventOnce error=${getErrorMessage(error)}`
          );
        }
      })
    );
