 ...
 * Can be overridden via `setConfig` to "recomplete" (retry tool calls) or "custom" (user-defined recovery) for alternative strategies.
 * @type {"flush" | "recomplete" | "custom"}
 */
GLOBAL_CONFIG.CC_RESQUE_STRATEGY = "flush";

/**
 * Updates the `GLOBAL_CONFIG` object with custom settings at runtime.
 * Merges provided config overrides into `GLOBAL_CONFIG`, enabling dynamic adjustment of system behavior (e.g., enabling debug logging for `ClientAgent`).
 * @param {Partial<typeof GLOBAL_CONFIG>} config - The partial configuration object to apply.
 * @returns {void}
 * @example
 * setConfig({ CC_LOGGER_ENABLE_DEBUG: true }); // Enables debug logging system-wide
 * setConfig({ CC_RESQUE_STRATEGY: "recomplete" }); // Changes recovery strategy
 */
...
/**
 * Exports the `GLOBAL_CONFIG` object, providing access to system-wide settings.
