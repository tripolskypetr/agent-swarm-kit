...
import { ICompletionSpec } from "src/interfaces/Completion.interface";

export class CompletionSpecService {
    readonly loggerService = inject<LoggerService>(TYPES.loggerService);

    private registry = new ToolRegistry<Record<string, ICompletionSpec>>("completionSpecService");

    public register = (key: string, value: ICompletionSpec) => {
        this.loggerService.log(`completionSpecService register`, { key });
        this.registry = this.registry.register(key, value);
    };

    public get = (key: string): ICompletionSpec => {
